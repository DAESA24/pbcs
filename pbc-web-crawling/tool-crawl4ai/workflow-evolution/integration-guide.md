...
### Example: Adding a New Fallback Strategy

**Extraction says:**
```yaml
fallback_strategies:
  - issue: "Documentation site deeper than expected"
    detection: "Max pages reached before content complete"
    resolution: "Increase max-pages parameter; consider targeting subsections"
    new_to_workflow: true
```

**Workflow update:**

Add to Fallback Strategies table:
```markdown
| Documentation site deeper than expected | Max pages reached before content complete | Increase max-pages parameter; consider targeting subsections |
```

### Example: Emphasizing a Pattern
...
In the relevant phase, add or update:
```markdown
> **Key Pattern:** Before executing any crawls, create a source-to-strategy
> mapping that documents which script and parameters to use for each source
> type. This prevents wasted effort from mismatched approaches.
```

### Example: Addressing a Workflow Gap

**Extraction says:**
```yaml
workflow_gaps:
  - gap: "No guidance on output organization structure"
    suggested_addition: "Add section on recommended directory structure"
    priority: "medium"
```
...
Organize crawled content by source category:
...
### Multiple Extractions for Same Workflow

When integrating multiple extractions:
1. Process in chronological order
2. Later extractions may supersede earlier recommendations
3. Update version number once after all integrations

### Conflicting Recommendations

If extractions have conflicting recommendations:
1. Note the conflict
2. Consult with user if significant
3. Prefer the most recent successful pattern
4. Document both approaches if both valid

### Workflow Already at "Mature" Status

For mature workflows:
- Still apply useful updates
- Be more conservative with changes
- Consider if change is refinement vs. restructure
- Bump minor version for refinements
