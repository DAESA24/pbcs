...
```bash
/c/Users/drewa/pbcs/media-download-pbc/.venv/Scripts/python.exe script.py
```
...
```bash
yt-dlp --config-location /c/Users/drewa/pbcs/media-download-pbc/configs/audio-only.conf <url>
```
...
- `web-crawling-pbc` - Both acquire content for RAG pipeline
